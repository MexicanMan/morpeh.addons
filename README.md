...
В плагин также входит доп. API для создания реквестов и событий на отдельных сущностях. Данные сущности не создаются каждый раз с нуля, а берутся из [пула](#entity-pool), поэтому их использование гораздо дешевле, чем создание через `World.CreateEntity()`.
